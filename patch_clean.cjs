...
// Normalize line endings to LF so that string replacements work predictably
code = code.replace(/\r\n/g, '\n');
...
// 4. Update useMemo
const shuffleTarget = `    const shuffledReading = shuffleArray(data.reading).slice(0, 5);`;
const shuffleReplace = `    const shuffledReading = shuffleArray(data.reading).slice(0, 5);
    const shuffledCloze = shuffleArray(data.cloze || []).slice(0, 5);`;
code = code.replace(shuffleTarget, shuffleReplace);
...
// Write back to file with proper line endings (optional, but keep it consistent)
